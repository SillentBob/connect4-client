...
public class Connect4Client : MonoBehaviour
{
    ...
    [FormerlySerializedAs("_Button")] [SerializeField] private GameObject      _ButtonPrefab;
    ...
    private void SpawnButtons()
    {
        ...
        for (int x = 0; x < 7; ++x)
        {
            ...
            RectTransform newButton = Instantiate(_ButtonPrefab, Vector3.zero, Quaternion.identity).GetComponent<RectTransform>();
    ...
    private void ButtonClicked(int x)
    {
        ...
        if (_Board[0][x] != -1)
        {
            Debug.Log("Can't put symbol there. Column is full.");
            return;
        }

        byte[] move = {(byte) MessageId.Move, (byte) x};
        Debug.Log($"Server.Send(move) {x}");
        _Server.Send(move);
    }

    private void ReceiveMessage(IAsyncResult ar)
    {
        int bytesCountReceived = _Server.EndReceive(ar);

        if (bytesCountReceived <= 0) 
            return;
        
        byte[] receivedData = new byte[bytesCountReceived];
        Buffer.BlockCopy(_ReceiveBuffer, 0, receivedData, 0, bytesCountReceived);
        ...
        UnityMainThreadDispatcher.Instance().Enqueue(() => ProcessBytesFromMessage(receivedData));
